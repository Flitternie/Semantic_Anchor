...
class CustomizedBartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.extra_intermediate_weighting = nn.parameter.Parameter(torch.ones(5), requires_grad=True)
        self.init_weights()
    ...
    def _resize_token_embeddings(self, new_num_tokens):
        ...
        if self.intermediate_lm_layer is not None and not self.config.tie_word_embeddings:
            old_lm_head = self.intermediate_lm_layer
            new_lm_head = self._get_resized_lm_head(old_lm_head, new_num_tokens)
            self.intermediate_lm_layer = new_lm_head
        if self.extra_lm_layer is not None and not self.config.tie_word_embeddings:
            old_lm_head = self.extra_lm_layer
            new_lm_head = self._get_resized_lm_head(old_lm_head, new_num_tokens)
            self.extra_lm_layer = new_lm_head
    ...
    def tie_weights(self):
        ...
        if self.intermediate_lm_layer is not None and self.config.tie_word_embeddings:
            self._tie_or_clone_weights(self.intermediate_lm_layer, self.get_input_embeddings())
            
        if self.extra_lm_layer is not None and self.config.tie_word_embeddings:
            self._tie_or_clone_weights(self.extra_lm_layer, self.get_input_embeddings())        
