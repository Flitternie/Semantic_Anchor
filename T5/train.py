...
from model import CustomizedT5ForConditionalGeneration
...
def train(args):
    ...
    val_loader = DataLoader(vocab_json, val_pt, args.batch_size // 4, training=False, hybrid=args.hybrid)
        ...
        for step, batch in enumerate(train_loader):
            ...
            if args.customized:
                if args.hybrid:
                    ...
                    extra_intermediate_labels = extra_intermediate_ids[:, :].clone()
                    extra_intermediate_labels[extra_intermediate_ids[:, :] == pad_token_id] = -100
                    extra_intermediate_masks = extra_intermediate_mask[:, :].clone()
                else:
                    assert len(batch) == 6
                    source_ids, source_mask, intermediate_ids, intermediate_mask, y = batch[0], batch[1], batch[2], batch[3], batch[4]
                intermediate_labels = intermediate_ids[:, :].clone()
                intermediate_labels[intermediate_ids[:, :] == pad_token_id] = -100
                intermediate_masks = intermediate_mask[:, :].clone()
        ...
        if args.customized:
            if args.aux_weighting == "dynamic":
                ...
                for i in range(args.sample_number):
                    ...
                    intermediate_labels = intermediate[:, :].clone()
                    intermediate_labels[intermediate[:, :] == pad_token_id] = -100
                    intermediate_masks = intermediate_mask[:, :].clone()
                    y_ids = y[:, :].contiguous()
                    labels = y[:, :].clone()
                    labels[y[:, :] == pad_token_id] = -100
...
def main():
    ...
    parser.add_argument('--adjustment_step', default=0.25, type=float)
