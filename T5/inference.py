...
def validate(args, model, data, device, tokenizer):
    ...
    if args.customized:
        ...
        if args.hybrid:
            ...
            logging.info(nn.functional.softmax(model.extra_intermediate_weighting).cpu().tolist())  # weighting for intermediate layers

    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            if args.customized:
                if args.hybrid:
                    source_ids, _, extra_intermediate_target_ids, _, intermediate_target_ids, _, target_ids, extra_ids = [
        ...
        if args.customized:
            all_intermediate_outputs = tokenizer.batch_decode(all_intermediate_outputs, skip_special_tokens=True,
            ...
            if args.hybrid:
                all_extra_intermediate_outputs = tokenizer.batch_decode(all_extra_intermediate_outputs,
...
def inference(args):
    ...
    try:
        ...
        raise Exception('Error loading config file')
    if args.customized:
        model = T5.from_pretrained(args.ckpt)
    else:
