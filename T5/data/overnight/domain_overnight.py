...
class OvernightDomain(Domain):
    ...
    def obtain_denotations(self, lf_list):
        ...
        for line in lf_list:
            tf.write(line + '\n')
        tf.flush()
        msg = subprocess.check_output(['./data/overnight/evaluator/overnight', self.dataset, tf.name])
        msg = msg.decode('utf8')
        tf.close()
        denotations = [
    ...
    def is_valid(self, ans):
        return not ('BADJAVA' in ans or 'ERROR' in ans or 'Exception' in ans or 'name what' in ans or ans is None)
