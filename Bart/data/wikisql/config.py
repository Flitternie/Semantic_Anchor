...
from Bart.data.wikisql.dbengine import DBEngine
...
def evaluate(args, outputs, targets, *xargs):
    ...
    db = DBEngine(os.path.join(args.input_dir, '{}.db'.format(data_split)))
    with open(os.path.join(args.input_dir, '{}.json'.format(data_split)), 'r') as f:
        data = json.load(f)
    ...
    with open(os.path.join(args.output_dir, "err_idxs.json"), "w") as f:
        json.dump(err_idxs, f)
